...
const trackOutline = `${trackPoints.map(([x, y], index) => `${index === 0 ? 'M' : 'L'}${x} ${y}`).join(' ')} Z`
    ...
    <div className="h-80 min-h-0 bg-panel-deep xl:h-full">
      <div className="flex h-full">
        <div className="w-28 shrink-0 overflow-y-auto bg-panel p-4 sm:w-32">
          <div className="mb-4 flex h-9 w-10 items-center justify-center bg-accent text-base font-extrabold">{current.rank}</div>
          <div className="space-y-3 font-extrabold">
            {details.map((detail) => (
              <div key={detail.label}>
                <div className="text-xs leading-tight">{detail.label}</div>
                <div className="mt-1 text-base leading-none text-accent">{detail.value}</div>
              </div>
            ))}
          </div>
        </div>
        <div className="flex min-w-0 flex-1 flex-col p-4">
          ...
          <div className="min-h-0 flex-1">
            <svg viewBox="0 0 280 280" className="h-full w-full">
              <path d={trackOutline} fill="none" strokeWidth={2} strokeLinejoin="round" className="stroke-accent" />
              {stats.map((stat, index) => {
                const isSelected = stat.player === current.player

                return (
                  <g key={stat.player} style={{ transform: `translate(${stat.x}px, ${stat.y}px)`, transition: 'transform 100ms linear' }}>
                    {isSelected ? <circle r={9} className="fill-accent/30" /> : null}
                    <image
                      href={isSelected ? carMarker : markers[index]}
                      x={isSelected ? -7 : -5}
                      y={isSelected ? -8 : -6}
